...
function closeView(){
	//Ti.API.info("app:navigationCloseView in ntCategory");
	
	Ti.App.removeEventListener("app:navigationCloseView", closeView);
	Ti.App.fireEvent("app:ntCategoryClosed");
};
...
function createCategoryBook (data) {
	...
	if (OS_IOS){
		...
		if (blankBook == "blank"){
			...
			var bookButton = Ti.UI.createButton({
				image: "/images/book_images/nt_"+(arrayValue+1)+".png",
				bookID: bookID,	
    ...
    Ti.App.addEventListener('app:ntCategoryClosed', removeEvents);   
    function removeEvents() {    	
    	Ti.App.removeEventListener('orient', bookButtonUpdate);
    	Ti.App.removeEventListener("app:ntCategoryClosed", removeEvents);  	
    };
...
// START CREATE ntCategory ScrollView
var ntCategory = Ti.UI.createScrollView({
	width:	Ti.UI.FILL,
	height: Ti.UI.FILL,
	left: 0,
	top: 0,
...
// START CREATE ntCategorySection	
var ntCategorySection = Ti.UI.createView({
	height: "50dp",
	width:	Ti.UI.FILL,
...
var ntCategorySectionLabel = Ti.UI.createLabel({
	text: L("nt_section"),
	textAlign: 'center',
	color: "white",
	font: {
...
ntCategorySection.add(ntCategorySectionLabel);
ntCategory.add(ntCategorySection);

var ntCategoryRow = Ti.UI.createView({
	width:	Ti.UI.SIZE,
	height: Ti.UI.SIZE,
...
for (var b=0; b<27; b++){
	...
	var bookData = { arrayValue: b,
					 bookID: padZero((+b + 40),2),	
					};		
	var categoryBook = createCategoryBook(bookData);
	
	ntCategoryRow.add(categoryBook);
	
};
// END LOOP - Create bookImages

ntCategory.add(ntCategoryRow);	
				
$.contentArea.add(ntCategory);